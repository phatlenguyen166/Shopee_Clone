...
const getCategories = async (req: Request, res: Response) => {
  ...
  const categories = await CategoryModel.find(condition).select({ __v: 0 }).lean()
  const response = {
...
const getCategory = async (req: Request, res: Response) => {
  const categoryDB = await CategoryModel.findById(req.params.category_id).select({ __v: 0 }).lean()
  if (categoryDB) {
    const response = {
...
const updateCategory = async (req: Request, res: Response) => {
  const { name } = req.body
  const categoryDB = await CategoryModel.findByIdAndUpdate(req.params.category_id, { name }, { new: true })
    .select({ __v: 0 })
    .lean()
  if (categoryDB) {
    const response = {
