...
const removeImageProduct = (image: any) => {
...
const addProduct = async (req: Request, res: Response) => {
  ...
  const { name, description, category, image, images, price, rating, price_before_discount, quantity, sold, view } =
    form
  const product = {
    name,
    description,
...
const getProducts = async (req: Request, res: Response) => {
  ...
  let [products, totalProducts]: [products: any, totalProducts: any] = await Promise.all([
    ProductModel.find(condition)
      ...
      .sort({ [sort_by]: order === 'desc' ? -1 : 1 } as Record<string, SortOrder>)
...
const getProduct = async (req: Request, res: Response) => {
  ...
  const productDB: any = await ProductModel.findOneAndUpdate(condition, { $inc: { view: 1 } }, { new: true })
    .populate('category')
    .select({ __v: 0 })
    .lean()
  if (productDB) {
...
const updateProduct = async (req: Request, res: Response) => {
  const form: Product = req.body
  const { name, description, category, image, rating, price, images, price_before_discount, quantity, sold, view } =
    form
  ...
  const productDB = await ProductModel.findByIdAndUpdate(req.params.product_id, product, {
    new: true
  })
    .select({ __v: 0 })
    .lean()
...
const deleteManyProducts = async (req: Request, res: Response) => {
  const list_id = (req.body.list_id as string[]).map((id: string) => new mongoose.Types.ObjectId(id))
